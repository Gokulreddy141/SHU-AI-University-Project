...
export default function SystemCheck({ onComplete, onSkip }: SystemCheckProps) {
    ...
    useEffect(() => {
        const runChecks = async () => {
            // Checks 1 & 2 share a single getUserMedia call to avoid double permission prompts
            ...
            try {
                ...
                updateCheck(1, { status: "fail", message: "Camera unavailable" });
            }

            // Check 2: Face visibility (frame brightness check using the same stream)
            if (cameraStream) {
                try {
                    ...
                    if (ctx) {
                        ...
                        if (avg > 20 && avg < 240) {
                            ...
                            updateCheck(1, { status: "warn", message: "Low lighting or camera covered" });
                        }
                    } else {
                    ...
                    updateCheck(1, { status: "warn", message: "Could not analyze camera frame" });
                } finally {
