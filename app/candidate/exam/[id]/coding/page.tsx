...
import { CodeEditorRenderer } from "@/components/features/CodeEditorRenderer";
...
export default function CandidateCodingPage() {
    ...
    const {
        ...
        timeLeftMs,
        isTabLocked
    } = useExamEngine(sessionId || "", examId, stageId || "", undefined, 60);
    ...
    if (loading && !questions.length) return <div className="p-12 text-center text-gray-400 animate-pulse">Loading workspace...</div>;
    if (!currentQuestion) return <div className="p-12 text-center text-gray-400 animate-pulse">Loading workspace...</div>;

    // Check which questions are completed based on having code populated
    const completedIndices = questions
        .map((q, idx) => (responses[q._id]?.submittedCode?.length ? idx : -1))
        .filter((idx) => idx !== -1);
    ...
    return (
        <>
            ...
            <div className={`flex h-[calc(100vh-64px)] w-full overflow-hidden bg-gray-950 text-white ${!isFullScreen ? 'blur-sm pointer-events-none' : ''}`}>
                {/* Sidebar Navigation */}
                <div className="w-64 border-r border-gray-800 bg-gray-900 p-6 flex flex-col justify-between">
                    <div>
                    ...
                    <div className="mt-8 border-t border-gray-800 pt-6 flex flex-col items-center">
                        ...
                        <p className="text-xs text-center text-gray-500 mt-3 px-2">
                            Code auto-saves locally and remotely.
                        </p>
                    </div>
                </div>
                ...
                <div className="flex-1 flex flex-col p-4 bg-black overflow-hidden relative">
                    <div className="absolute top-8 right-8 flex gap-3 z-10">
                        <button
                            ...
                            className="px-4 py-1.5 rounded bg-gray-800 text-gray-300 font-medium text-xs disabled:opacity-30 hover:bg-gray-700 transition"
                        >
                            &larr; Prev
                        </button>
                        <button
                            ...
                            disabled={isLast}
                            className="px-4 py-1.5 flex gap-2 rounded bg-gray-800 text-gray-300 font-medium text-xs disabled:opacity-30 hover:bg-gray-700 transition"
                        >
                            Next &rarr;
                        </button>
                    </div>
                    ...
                    {currentQuestion.type === "CODING" ? (
                        <div className="flex-1 min-h-0 w-full mb-4 mt-12 px-4 pb-2">
                            <CodeEditorRenderer question={currentQuestion} sessionId={sessionId} />
                        </div>
                    ) : (
                        <div className="text-center mt-32 p-8 border border-gray-800 rounded bg-gray-900 text-gray-400 max-w-lg mx-auto">
                            Unsupported question type &apos;{currentQuestion.type}&apos; rendering in Coding view.
                        </div>
                    )}
                </div>
            </div>
